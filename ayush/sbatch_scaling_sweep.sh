...
#SBATCH --output=/scratch/ak13124/attention_dilution/logs/scaling_sweep_%j.out
#SBATCH --error=/scratch/ak13124/attention_dilution/logs/scaling_sweep_%j.err
...
# (run sbatch_refusal_pipeline.sh at least once, or unskip just the extract
# stage in that file).
#
# This script reuses the venv created by sbatch_refusal_pipeline.sh if
# present; otherwise it creates it the same way.
#
# Submit from anywhere:
#   sbatch /scratch/ak13124/attention_dilution/ayush/sbatch_scaling_sweep.sh
...
if [ \"\$missing\" = \"1\" ]; then
  ...
  echo \"run sbatch_refusal_pipeline.sh first (or at least its extract stage).\" >&2
  exit 1
fi
...
echo \"=== scaling_sweep | job \${SLURM_JOB_ID:-local} ===\"
echo \"Repo: \$(pwd)\"
